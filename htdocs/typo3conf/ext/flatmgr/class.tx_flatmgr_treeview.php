...
*  Copyright notice
*
*  (c)  2009 Joachim Ruhs <postmaster@joachim-ruhs.de>
...
class tx_flatmgr_treeview {
	 ...
	 */
	function user_displayGroupTree($PA, $fobj) {
		$table = $PA['table'];
		$field = $PA['field'];
		$row   = $PA['row'];
				...
				if($config['treeView'] && $config['foreign_table']) {
					...
					if($config['treeViewClass'] && is_object($treeViewObj = &t3lib_div::getUserObj($config['treeViewClass'],'user_',false))) {
			            ...
			            if (version_compare(TYPO3_branch, '6.0', '<')) {
			                $treeViewObj = t3lib_div::makeInstance('tx_flatmgr_tceFunc_selectTreeView');
			            } else {
			                $treeViewObj = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('tx_flatmgr_tceFunc_selectTreeView');
			            }
					}
...
//debug($this->NA_Items.implode($errorMsg,chr(10)).$item);
	...
	function findRecursiveGroups ($PA,$row,$table,$storagePid,$treeIds) {
		$errorMsg = array();
		if ($table == 'tt_content' && $row['CType']=='list' && $row['list_type']=='tt_address_pi1') { // = tt_content element which inserts plugin tx_flatmgr_pi1
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/flatmgr/class.tx_flatmgr_treeview.php'])    {
    include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/flatmgr/class.tx_flatmgr_treeview.php']);
}
?>
