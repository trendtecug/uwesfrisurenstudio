...
var $document = $(document),
    ...
    $isOpenTimeBg = $('#isOpenTime-bg'),
    $impressum = $('#impressum'),
    $impressumBg = $('#impressum-bg'),
    $logo = $('#logo'),
    $loaderBg = $('#loader-bg'),
    $nav = $('#nav'),
    $navItems = $('.nav-item'),
    ...
    $btnImpressum = $('#btn-impressum-open'),
    $closeImpressum = $('#btn-impressum-close'),
    $btnIsOpenTime = $('#btn-isOpenTime-open'),
    $closeIsOpenTime = $('#btn-isOpenTime-close'),
    $btns = $('.btn'),
    $btnsBGs = $('.btn-bg-mask'),
    $btnNext = $('#btn-next'),
    $gtuNext = $('#gtu-next'),
...
$(document).ready(function () {
    ...
    window.$main = $('.main'),
        ...
        window.$impressumBg = ('#impressum-bg'),
        window.$datenschutz = $('#datenschutz'),
        window.$datenschutzBg = ('#datenschutz-bg'),
        window.$isOpenTime = $('#isOpenTime'),
        window.$isOpenTimeBg = ('#isOpenTime-bg'),
        window.$logo = $('#logo'),
        ...
        window.$btnImpressum = $('#btn-impressum-open'),
        window.$closeImpressum = $('#btn-impressum-close'),
        window.$btnDatenschutz = $('#btn-datenschutz-open'),
        window.$closeDatenschutz = $('#btn-datenschutz-close'),
        window.$btnIsOpenTime = $('#btn-isOpenTime-open'),
        window.$closeIsOpenTime = $('#btn-isOpenTime-close'),

        window.$btns = $('.btn'),
        window.$btnsBGs = $('.btn-bg-mask'),
        window.$btnNext = $('#btn-next'),
        window.$gtuNext = $('#gtu-next'),
    ...
    window.animation_possible = true;
...
app.init = function (parent, children) {
    if ((Detectizr.browser.name == "ie" && parseInt(Detectizr.browser.version) <= 11) || (Detectizr.browser.name == "firefox" && Detectizr.os.name == "mac os")) {
        window.animation_possible = false;
        $html.addClass("animation-disabled");
        $('.csc-default_1').css('visibility', 'hidden');
    } else {
        $html.addClass("animation-enabled");
    }


    TweenLite.to($gtuNext, 0.01, {opacity: '0'});
    ...
    app.imprint.init();
    app.datenschutz.init();
    app.isOpenTime.init();
    $btnImpressum.on('click', function (e) {
        app.imprint.open();
    });
    $btnDatenschutz.on('click', function (e) {
        app.datenschutz.open();
    });
    $btnIsOpenTime.on('click', function (e) {app.isOpenTime.open();});
    $closeImpressum.on('click', function (e) {
        app.imprint.close();
    });
    $closeDatenschutz.on('click', function (e) {
        app.datenschutz.close();
    });
    $closeIsOpenTime.on('click', function (e) {app.isOpenTime.close();});
...
//        app.paper.init();
        ...
        $gtuNext.on('click', function () {
            if (!$html.hasClass("is-locked")) {
                slider.slide($('#gtue-vertragspartner').index(), 'next');
            }
        });
...
//            app.paper.resize();
            ...
            if (pathname == "" || pathname == $($navLinks[0]).attr('href'))
                TweenLite.to($gtuNext, 0.01, {opacity: '1'});
            else
                TweenLite.to($gtuNext, 0.01, {opacity: '0'});
            var d = $('.is-active .cell');
            var d3 = $($('.is-active .right')[0]);
            var d2 = d[0].lastElementChild;

            var $h = d2.offsetHeight + d2.offsetTop;
            var posYnew = $h;
            if (Detectizr.browser.name == "ie")
                posYnew += +parseInt(d3.css('padding-top'));
            var $windowhalf = ($(window).width() / 2);
            var $left = d3.css('padding-left');
            TweenLite.to($gtuNext, 0.01, {left: parseInt($left) + $windowhalf, top: posYnew});

            if (desktop) {
                $('.project-mask').css('display', 'block');
            } else {
                $('.project-mask').css('display', 'none');
            }
            if ($window.outerWidth() < 1025) {
                $document.unbind('mousewheel DOMMouseScroll', app.scrollNav);
            } else {
        ...
        $gtuNext.find('.btn-bg').css('padding-top', '0px');
        $gtuNext.on('click', function () {
            $('html, body').animate({scrollTop: ($('#gtue-vertragspartner').offset().top)}, 'slow');
        });
...
app.loader = function () {
    ...
    if (desktop) {
        ...
        var max2 = max / 85;
        TweenLite.set($loaderBg, {scaleX: max2, scaleY: max2});
    }
    if (!Modernizr.csstransitions) {
        TweenLite.to($loaderBg, 2, {
            scaleX: 0.01, scaleY: 0.01, ease: Expo.EaseIn, delay: 2, onComplete: function () {
                setTimeout(function () {
                    $html.addClass('is-loaded');
                    setTimeout(function () {
                        $html.addClass('is-loaded-bg');
                    }, 1000);
                }, 1000);
            }
        });
    ...
    setTimeout(function () {
        ...
        if (desktop) {
            ...
            var $windowhalf = ($(window).width() / 2);
            var $left = d3.css('padding-left'); //d2.offsetLeft;
            TweenLite.to($gtuNext, 0.01, {left: parseInt($left) + $windowhalf, top: posYnew});
        } else {
            $gtuNext.find('.btn-bg').css('padding-top', '0px');
        }
...
slider.init = function () {
    ...
    TweenLite.set($dotClone, {scaleX: 1.0, scaleY: 1.0, top: _dotPos.top + 'px', bottom: _dotPos.bottom});
    TweenLite.set($dots, {scaleX: 1.0, scaleY: 1.0});
    TweenLite.set($dot, {scaleX: 1, scaleY: 1});

    _prevColors = {
        bg: $overlay.data('bg') === undefined ? null : $overlay.data('bg'),
        bg_btn: $overlay.data('bg_btn') === undefined ? null : $overlay.data('bg_btn'),
        pattern: $overlay.data('pattern') === undefined ? null : app.hexToRgb($overlay.data('pattern')),
        main: $overlay.data('main') === undefined ? null : app.hexToRgb($overlay.data('main')),
        secondary: $overlay.data('secondary') === undefined ? null : app.hexToRgb($overlay.data('secondary')),
        siteTitle: $overlay.data('sitetitle') === undefined ? null : $overlay.data('sitetitle'),
        btnContact: $overlay.data('btncontact') === undefined ? null : $overlay.data('btncontact'),
        logo: $overlay.data('logo') === undefined ? null : $overlay.data('logo')
    };
...
slider.slide = function (index, direction) {
    ...
    _colors = {
        ...
        main: $overlay.data('main') === undefined ? null : app.hexToRgb($overlay.data('main')),
        secondary: $overlay.data('secondary') === undefined ? null : app.hexToRgb($overlay.data('secondary')),
        siteTitle: $overlay.data('sitetitle') === undefined ? '#fff' : $overlay.data('sitetitle'),
        btnContact: $overlay.data('btncontact') === undefined ? '#fff' : $overlay.data('btncontact'),
        logo: $overlay.data('logo') === undefined ? null : $overlay.data('logo')
    };
...
//    app.paper.updateColors(_colors);
    ...
    if (!$link.data('active')) {
        ...
        TweenLite.to($prevDot, 0.4, {delay: 0.4, scaleX: 1.0, scaleY: 1.0, ease: Expo.easeOut});
...
slider.updateColors = function (colors, direction) {
    if (direction === null) {
        ...
        $btnsBGs.css('background', colors.bg_btn);
        $btnNext.css('background', colors.bg);
        $btnNext.find('svg').css('fill', colors.btnContact);
        TweenLite.to($('#logo-font'), 1, {delay: 1, color: colors.logo});
    } else {
        if (!Modernizr.csstransitions) {
        ...
        TweenLite.to($btnsBGs, 1, {backgroundColor: colors.bg_btn});
        ...
        TweenLite.to(
            ...
            {css: {color: colors.logo}, ease: Back.easeOut}
        );
    }
};
...
app.contact = {
    ...
    open: function () {
        if (!$html.hasClass("is-locked")) {
            $('#email').attr("placeholder_org", $('#email').attr('placeholder'));
            $('#url').attr("placeholder_org", $('#url').attr('placeholder'));
            $('#phone').attr("placeholder_org", $('#phone').attr('placeholder'));
            $('#zip').attr("placeholder_org", $('#zip').attr('placeholder'));
    ...
    close: function () {
        ...
        $('#email').attr("placeholder", $('#email').attr('placeholder_org'));
        $('#url').attr("placeholder", $('#url').attr('placeholder_org'));
        $('#phone').attr("placeholder", $('#phone').attr('placeholder_org'));
        $('#zip').attr("placeholder", $('#zip').attr('placeholder_org'));
        $("#newContactForm .validate").each(function () {
...
app.imprint = {
    init: function () {
        var btnImpressum = $btnImpressum[0].getBoundingClientRect(),
            pathname = window.location.hash.replace(/\//g, '');
        TweenLite.set($impressumBg, {
            top: btnImpressum.top,
            right: viewWidth * 2 - btnImpressum.left - btnImpressum.width, bottom: viewHeight - btnImpressum.top - 4,
            left: btnImpressum.left - btnImpressum.width
        });
    ...
    open: function () {
        if (!$html.hasClass("is-locked")) {
            ...
            if (desktop) {
                $document.unbind('keyup', app.keyNav);
                $document.bind('keyup', app.imprint.keyNav);
                $document.unbind('mousewheel DOMMouseScroll', app.scrollNav);
                History.replaceState('', 'KFZ Schupp', '/#impressum');
            }
            $html.addClass('is-locked');
            $impressum.data('open', true).addClass('is-open');
            $btnImpressum.addClass('is-open');
        }
    },
    close: function () {
        if (desktop) {
            $document.bind('keyup', app.keyNav);
            $document.unbind('keyup', app.imprint.keyNav);
            if ($window.outerWidth() >= 1025) {
                $document.bind('mousewheel DOMMouseScroll', app.scrollNav);
            }
            var $currentProject = $nav.find('li .is-active');
        ...
        $html.removeClass('is-locked');
        $impressum.data('open', false).removeClass('is-open');
        $btnImpressum.removeClass('is-open');
    },
    keyNav: function (event) {
        if (event.which === 27) {
            app.imprint.close();
...
app.datenschutz = {
    init: function () {
        var btnDatenschutz = $btnDatenschutz[0].getBoundingClientRect(),
            pathname = window.location.hash.replace(/\//g, '');
        TweenLite.set($datenschutzBg, {
            top: btnDatenschutz.top,
            right: viewWidth * 2 - btnDatenschutz.left - btnDatenschutz.width, bottom: viewHeight - btnDatenschutz.top - 4,
            left: btnDatenschutz.left - btnDatenschutz.width
        });
    ...
    open: function () {
        if (!$html.hasClass("is-locked")) {
            if (desktop) {
                ...
                $document.bind('keyup', app.imprint.keyNav);
                $document.unbind('mousewheel DOMMouseScroll', app.scrollNav);
                History.replaceState('', 'KFZ Schupp', '/#datenschutz');
            }
            $html.addClass('is-locked');
            $datenschutz.data('open', true).addClass('is-open');
            $btnDatenschutz.addClass('is-open');
        }
    },
    close: function () {
        if (desktop) {
        ...
        $html.removeClass('is-locked');
        $datenschutz.data('open', false).removeClass('is-open');
        $btnDatenschutz.removeClass('is-open');
    },
    keyNav: function (event) {
        if (event.which === 27) {
            app.datenschutz.close();
...
app.isOpenTime = {
    init: function () {
        var btnisOpenTime = $btnIsOpenTime[0].getBoundingClientRect(),
            pathname = window.location.hash.replace(/\//g, '');
        TweenLite.set($isOpenTimeBg, {
            top: btnisOpenTime.top,
            right: viewWidth * 2 - btnisOpenTime.left - btnisOpenTime.width, bottom: viewHeight - btnisOpenTime.top - 4,
            left: btnisOpenTime.left - btnisOpenTime.width
        });
        if (pathname === '#öffnunfszeiten') {
            app.isOpenTime.open();
        }
    },
    open: function () {
        if (!$html.hasClass("is-locked")) {
            if (desktop) {
                ...
                $document.bind('keyup', app.imprint.keyNav);
                $document.unbind('mousewheel DOMMouseScroll', app.scrollNav);
                History.replaceState('', 'KFZ Schupp', '/#öffnungszeiten');
            }
            $html.addClass('is-locked');
            $isOpenTime.data('open', true).addClass('is-open');
            $btnIsOpenTime.addClass('is-open');
        }
    },
    close: function () {
        if (desktop) {
        ...
        $html.removeClass('is-locked');
        $isOpenTime.data('open', false).removeClass('is-open');
        $btnIsOpenTime.removeClass('is-open');
    },
    keyNav: function (event) {
        if (event.which === 27) {
            app.isOpenTime.close();
        }
    }
};
